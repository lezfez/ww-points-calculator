-- JSONB category model for recipes.
-- Run once in Supabase SQL editor.
--
-- Goal:
-- 1) Add public.recipes.kategorien (jsonb array of slugs)
-- 2) Add controlled category catalog public.recipe_categories
-- 3) Backfill from legacy public.recipes.kategorie (text)
-- 4) Enforce validation + add GIN index for fast filtering

begin;

-- 1) Controlled category catalog
...
-- Helper: ensure a JSONB value is an array of strings
...
-- 3) Ensure any legacy categories that are not in seed list are auto-added to catalog
with derived as (
...
-- Backfill recipes.kategorien from legacy recipes.kategorie
...
-- 4) Validation trigger: normalize array + enforce catalog membership
...
begin
  ...
  -- Normalize: trim, slugify, dedupe, stable order
  new.kategorien := (
    select coalesce(jsonb_agg(slug order by slug), '[]'::jsonb)
    from (
...
-- Optional but recommended: fast filtering with @>
...
-- Example filters:
-- all desserts:
-- select * from public.recipes where kategorien @> '["dessert"]'::jsonb;
--
-- all vegetarian main dishes:
-- select * from public.recipes
-- where kategorien @> '["vegetarisch","hauptspeise"]'::jsonb;
